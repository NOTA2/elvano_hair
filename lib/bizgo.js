import { getBaseUrl, getBizgoSenderKey, requireEnv } from "@/lib/config";
import { updateNotificationTemplate } from "@/lib/db";
import { fillTemplate } from "@/lib/documents";
import { buildTemplateValues } from "@/lib/templateVariables";

const TEMPLATE_SYNC_COOLDOWN_MS = 1000 * 60;

function getBizgoBaseUrl() {
  return "https://mars.ibapi.kr/api/comm";
}

function buildBizgoUrl(path, query) {
  const url = new URL(String(path || "").replace(/^\/+/, ""), `${getBizgoBaseUrl()}/`);

  Object.entries(query || {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") {
      url.searchParams.set(key, String(value));
    }
  });

  return url;
}

function getBizgoAuthorizationValue() {
  const apiKey = requireEnv("BIZGO_API_KEY").trim();

  if (/^bearer\s+/i.test(apiKey)) {
    return apiKey;
  }

  return apiKey.replace(/^apikey\s+/i, "");
}

function getBizgoHeaders(contentType = "application/json") {
  return {
    Authorization: getBizgoAuthorizationValue(),
    "Content-Type": contentType
  };
}

function createBizgoApiError(status, payload) {
  const error = new Error(`Bizgo API 실패: ${status} ${JSON.stringify(payload)}`);
  error.name = "BizgoApiError";
  error.bizgoStatus = Number(status) || 0;
  error.bizgoPayload = payload || null;
  return error;
}

async function bizgoRequest(path, { method = "GET", query, body } = {}) {
  const url = buildBizgoUrl(path, query);

  const response = await fetch(url.toString(), {
    method,
    headers: getBizgoHeaders(),
    body: body ? JSON.stringify(body) : undefined,
    cache: "no-store"
  });

  const data = await response.json().catch(() => null);

  if (!response.ok) {
    throw createBizgoApiError(response.status, data);
  }

  return data;
}

function sleep(ms) {
  return new Promise((resolve) => {
    setTimeout(resolve, ms);
  });
}

function getBizgoDestinationResults(payload) {
  const candidates = [
    payload?.data?.data?.destinations,
    payload?.data?.destinations,
    payload?.destinations
  ];

  return candidates.find(Array.isArray) || [];
}

function getBizgoInquiryMessages(payload) {
  const candidates = [
    payload?.data?.data?.messages,
    payload?.data?.messages,
    payload?.messages
  ];

  return candidates.find(Array.isArray) || [];
}

function getBizgoFirstDestination(payload) {
  return getBizgoDestinationResults(payload)[0] || null;
}

function buildBizgoFailureMessage(code, fallbackText) {
  const normalizedCode = toNullableString(code);
  const normalizedText = toNullableString(fallbackText);

  if (normalizedCode && normalizedText) {
    return `알림톡 발송 실패로 확인되었습니다. (${normalizedCode} ${normalizedText})`;
  }

  if (normalizedText) {
    return `알림톡 발송 실패로 확인되었습니다. (${normalizedText})`;
  }

  if (normalizedCode) {
    return `알림톡 발송 실패로 확인되었습니다. (${normalizedCode})`;
  }

  return "알림톡 발송 실패로 확인되었습니다.";
}

function parseBizgoInquiryMessage(message) {
  if (!message) {
    return {
      status: "requested",
      message: "알림톡 접수는 완료되었지만 최종 발송 결과는 아직 확인 중입니다."
    };
  }

  if (message.responseCode && message.responseCode !== "A000") {
    return {
      status: "failed",
      message: buildBizgoFailureMessage(message.responseCode, message.responseText || "접수 실패")
    };
  }

  if (!message.reportCode) {
    return {
      status: "requested",
      message: "알림톡 접수는 완료되었지만 최종 발송 결과는 아직 확인 중입니다."
    };
  }

  if (message.reportCode === "10000") {
    return {
      status: "sent",
      message: "알림톡 발송이 완료되었습니다."
    };
  }

  return {
    status: "failed",
    message: buildBizgoFailureMessage(message.reportCode, message.reportText || "최종 발송 실패")
  };
}

function classifyBizgoInquiryMessage(message) {
  const result = parseBizgoInquiryMessage(message);

  if (result.status === "failed") {
    throw new Error(result.message);
  }

  return result;
}

export function extractBizgoMsgKey(bizgoResponse) {
  const candidates = [
    bizgoResponse?.msgKey,
    bizgoResponse?.message?.msgKey,
    bizgoResponse?.send?.data?.data?.destinations?.[0]?.msgKey,
    bizgoResponse?.send?.data?.destinations?.[0]?.msgKey,
    bizgoResponse?.send?.destinations?.[0]?.msgKey,
    bizgoResponse?.inquiry?.data?.data?.messages?.[0]?.msgKey,
    bizgoResponse?.inquiry?.data?.messages?.[0]?.msgKey,
    bizgoResponse?.inquiry?.messages?.[0]?.msgKey
  ];

  return candidates
    .map((value) => toNullableString(value))
    .find(Boolean) || null;
}

function assertBizgoSendSuccess(payload) {
  const authCode = payload?.common?.authCode;
  const dataCode = payload?.data?.code;
  const dataResult = payload?.data?.result;
  const destinations = getBizgoDestinationResults(payload);

  if (authCode && authCode !== "A000") {
    throw new Error(
      `Bizgo 발송 실패: ${authCode} ${payload?.common?.authResult || "인증 오류"}`
    );
  }

  if (dataCode && dataCode !== "A000") {
    throw new Error(`Bizgo 발송 실패: ${dataCode} ${dataResult || "요청 실패"}`);
  }

  if (destinations.length === 0) {
    throw new Error("Bizgo 발송 실패: 접수 결과(destinations)가 없습니다.");
  }

  const failedDestination = destinations.find((destination) => destination?.code !== "A000");

  if (failedDestination) {
    throw new Error(
      `Bizgo 발송 실패: ${failedDestination.code || "UNKNOWN"} ${failedDestination.result || "수신 결과 실패"}`
    );
  }
}

function looksLikeAlimtalkTemplate(candidate) {
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    return false;
  }

  return Boolean(
    candidate.templateCode ||
      candidate.templateName ||
      candidate.senderKey ||
      candidate.text
  );
}

function extractAlimtalk(payload) {
  const candidates = [
    payload?.data?.data?.alimtalk,
    payload?.data?.alimtalk,
    payload?.data?.data,
    payload?.data,
    payload?.alimtalk,
    payload
  ];

  return candidates.find(looksLikeAlimtalkTemplate) || null;
}

function toNullableString(value) {
  if (value === undefined || value === null) {
    return null;
  }

  const normalized = String(value).trim();
  return normalized ? normalized : null;
}

function stripProtocol(url) {
  if (!url) {
    return "";
  }

  try {
    const parsed = new URL(String(url));
    return `${parsed.host}${parsed.pathname}${parsed.search}${parsed.hash}`;
  } catch {
    return String(url).replace(/^https?:\/\//i, "");
  }
}

function sanitizeReplaceWords(values) {
  return Object.fromEntries(
    Object.entries(values).filter(([, value]) => value !== undefined && value !== null && value !== "")
  );
}

function resolveTemplateTextForSend(text, values) {
  if (!text) {
    return text;
  }

  return fillTemplate(String(text), values);
}

function isPlainObject(value) {
  return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}

function resolveTemplateValueForSend(value, values) {
  if (value === undefined || value === null) {
    return undefined;
  }

  if (typeof value === "string") {
    const resolved = resolveTemplateTextForSend(value, values);
    return String(resolved).trim() ? resolved : undefined;
  }

  if (Array.isArray(value)) {
    const resolvedValues = value
      .map((item) => resolveTemplateValueForSend(item, values))
      .filter((item) => item !== undefined);

    return resolvedValues.length > 0 ? resolvedValues : undefined;
  }

  if (isPlainObject(value)) {
    const resolvedEntries = Object.entries(value)
      .map(([key, item]) => [key, resolveTemplateValueForSend(item, values)])
      .filter(([, item]) => item !== undefined);

    if (resolvedEntries.length === 0) {
      return undefined;
    }

    return Object.fromEntries(resolvedEntries);
  }

  return value;
}

function templateValuesFromDocument(document) {
  return buildTemplateValues({
    branch_name: document.branch_name,
    branch_phone: document.branch_phone,
    document_title: document.document_title,
    document_date: document.document_date,
    customer_name: document.customer_name,
    phone_last4: document.phone_last4,
    recipient_phone: document.recipient_phone,
    designer_name: document.designer_name,
    created_at: document.created_at,
    limit_date: document.limit_date,
    document_url: stripProtocol(`${getBaseUrl()}/s/${document.token}`)
  });
}

function canUseApprovedTemplateSnapshot(template) {
  if (!template || template.inspection_status !== "APR") {
    return false;
  }

  return Boolean(
    template.template_code &&
      template.sender_key &&
      template.message &&
      template.last_synced_at
  );
}

function hasFreshInspectionSnapshot(template, maxAgeMs = TEMPLATE_SYNC_COOLDOWN_MS) {
  if (!template?.inspection_status || !template.last_synced_at) {
    return false;
  }

  const syncedAt = new Date(template.last_synced_at);

  if (Number.isNaN(syncedAt.getTime())) {
    return false;
  }

  return Date.now() - syncedAt.getTime() < maxAgeMs;
}

function buildInspectionStatusError(template) {
  return new Error(
    `알림톡 템플릿이 아직 승인 완료 상태가 아닙니다. 현재 상태: ${template?.inspection_status || "미확인"}`
  );
}

function assertApprovedNotificationTemplate(template) {
  if (template?.inspection_status !== "APR") {
    throw buildInspectionStatusError(template);
  }
}

function buildButtonFromTemplate(template, values, { fillValues = true } = {}) {
  if (!template.button_name || !template.button_type) {
    return null;
  }

  const button = {
    name: template.button_name,
    type: template.button_type
  };

  if (template.button_url_mobile) {
    button.urlMobile = fillValues
      ? fillTemplate(template.button_url_mobile, values)
      : resolveTemplateTextForSend(template.button_url_mobile, values);
  }

  if (template.button_url_pc) {
    button.urlPc = fillValues
      ? fillTemplate(template.button_url_pc, values)
      : resolveTemplateTextForSend(template.button_url_pc, values);
  }

  if (template.button_scheme_ios) {
    button.schemeIos = fillValues
      ? fillTemplate(template.button_scheme_ios, values)
      : resolveTemplateTextForSend(template.button_scheme_ios, values);
  }

  if (template.button_scheme_android) {
    button.schemeAndroid = fillValues
      ? fillTemplate(template.button_scheme_android, values)
      : resolveTemplateTextForSend(template.button_scheme_android, values);
  }

  if (template.button_tel_number) {
    button.telNumber = fillValues
      ? fillTemplate(template.button_tel_number, values)
      : resolveTemplateTextForSend(template.button_tel_number, values);
  }

  return button;
}

function buildBizgoTemplateSnapshot(template) {
  const remoteSnapshot = extractAlimtalk(template?.remote_payload);

  if (remoteSnapshot) {
    return remoteSnapshot;
  }

  if (looksLikeAlimtalkTemplate(template?.remote_payload)) {
    return template.remote_payload;
  }

  const button = buildButtonFromTemplate(template, {}, { fillValues: false });
  const snapshot = {
    senderKey: template.sender_key,
    senderKeyType: template.sender_key_type || "S",
    templateCode: template.template_code,
    templateName: template.template_name,
    templateMessageType: template.template_message_type || "BA",
    templateEmphasizeType: template.template_emphasize_type || "NONE",
    categoryCode: template.category_code,
    securityFlag: template.security_flag ? "true" : undefined,
    text: template.message,
    title: template.title,
    subTitle: template.subtitle,
    header: template.header
  };

  if (button) {
    snapshot.attachment = {
      button: [button]
    };
  }

  return snapshot;
}

function pickBizgoSendTemplateFields(snapshot) {
  const allowedKeys = [
    "senderKey",
    "templateCode",
    "text",
    "title",
    "subTitle",
    "header",
    "price",
    "currencyType",
    "link",
    "attachment",
    "supplement"
  ];

  return Object.fromEntries(
    allowedKeys
      .map((key) => [key, snapshot?.[key]])
      .filter(([, value]) => value !== undefined)
  );
}

function buildBizgoSendAlimtalk(template, values) {
  const snapshot = buildBizgoTemplateSnapshot(template);
  const resolvedSnapshot = resolveTemplateValueForSend(snapshot, values) || {};
  const emphasizeType =
    resolvedSnapshot.templateEmphasizeType ||
    template.template_emphasize_type ||
    "NONE";
  const text =
    resolvedSnapshot.text !== undefined
      ? resolvedSnapshot.text
      : resolveTemplateTextForSend(template.message, values);
  const sendTemplate = pickBizgoSendTemplateFields({
    ...resolvedSnapshot,
    senderKey: resolvedSnapshot.senderKey || template.sender_key || getBizgoSenderKey(),
    templateCode: resolvedSnapshot.templateCode || template.template_code,
    msgType: emphasizeType === "IMAGE" ? "AI" : "AT",
    ...(text ? { text } : {})
  });

  return {
    ...sendTemplate,
    msgType: emphasizeType === "IMAGE" ? "AI" : "AT"
  };
}

function countTemplateStructureItems(value) {
  if (Array.isArray(value)) {
    return value.length;
  }

  return value ? 1 : 0;
}

export function summarizeBizgoTemplateStructure(template) {
  const snapshot = buildBizgoTemplateSnapshot(template);
  const parts = [];
  const emphasizeType =
    snapshot?.templateEmphasizeType || template?.template_emphasize_type || "NONE";

  parts.push(emphasizeType === "IMAGE" ? "이미지형" : "텍스트형");

  const buttonCount = countTemplateStructureItems(snapshot?.attachment?.button);
  const quickReplyCount = countTemplateStructureItems(snapshot?.supplement?.quickReply);

  if (buttonCount > 0) {
    parts.push(`버튼 ${buttonCount}개`);
  }

  if (quickReplyCount > 0) {
    parts.push(`바로연결 ${quickReplyCount}개`);
  }

  if (snapshot?.link) {
    parts.push("대표링크 있음");
  }

  if (snapshot?.imgName || snapshot?.imgUrl) {
    parts.push("이미지 있음");
  }

  if (snapshot?.attachment?.item) {
    parts.push("아이템 있음");
  }

  if (snapshot?.attachment?.itemHighlight) {
    parts.push("강조영역 있음");
  }

  return parts.join(" · ");
}

function buildNotificationTemplateBody(template, { fillValues } = {}) {
  const values = fillValues || {};
  const button = buildButtonFromTemplate(template, values);
  const alimtalk = {
    senderKey: template.sender_key,
    senderKeyType: template.sender_key_type || "S",
    templateCode: template.template_code,
    templateName: template.template_name,
    templateMessageType: template.template_message_type || "BA",
    templateEmphasizeType: template.template_emphasize_type || "NONE",
    text: fillValues ? fillTemplate(template.message, values) : template.message
  };

  if (template.category_code) {
    alimtalk.categoryCode = template.category_code;
  }

  if (template.security_flag) {
    alimtalk.securityFlag = "true";
  }

  if (template.title) {
    alimtalk.title = fillValues ? fillTemplate(template.title, values) : template.title;
  }

  if (template.subtitle) {
    alimtalk.subTitle = fillValues
      ? fillTemplate(template.subtitle, values)
      : template.subtitle;
  }

  if (template.header) {
    alimtalk.header = fillValues ? fillTemplate(template.header, values) : template.header;
  }

  if (button) {
    alimtalk.attachment = {
      button: [button]
    };
  }

  return { alimtalk };
}

export function mapBizgoTemplateToLocal(apiTemplate) {
  const firstButton = apiTemplate?.attachment?.button?.[0] || null;

  return {
    sender_key: toNullableString(apiTemplate?.senderKey) || getBizgoSenderKey(),
    sender_key_type: toNullableString(apiTemplate?.senderKeyType) || "S",
    template_code: toNullableString(apiTemplate?.templateCode),
    template_name: toNullableString(apiTemplate?.templateName),
    template_message_type: toNullableString(apiTemplate?.templateMessageType) || "BA",
    template_emphasize_type:
      toNullableString(apiTemplate?.templateEmphasizeType) || "NONE",
    category_code: toNullableString(apiTemplate?.categoryCode),
    security_flag:
      String(apiTemplate?.securityFlag || "").toLowerCase() === "true",
    message: toNullableString(apiTemplate?.text) || "",
    title: toNullableString(apiTemplate?.title),
    subtitle: toNullableString(apiTemplate?.subTitle),
    header: toNullableString(apiTemplate?.header),
    button_name: toNullableString(firstButton?.name),
    button_type: toNullableString(firstButton?.type),
    button_url_mobile: toNullableString(firstButton?.urlMobile),
    button_url_pc: toNullableString(firstButton?.urlPc || firstButton?.urlPC),
    button_scheme_ios: toNullableString(firstButton?.schemeIos),
    button_scheme_android: toNullableString(firstButton?.schemeAndroid),
    button_tel_number: toNullableString(firstButton?.telNumber),
    inspection_status: toNullableString(apiTemplate?.inspectionStatus),
    remote_block: String(apiTemplate?.block || "").toLowerCase() === "true",
    remote_dormant: String(apiTemplate?.dormant || "").toLowerCase() === "true",
    remote_created_at: toNullableString(apiTemplate?.createAt),
    remote_modified_at: toNullableString(apiTemplate?.modifiedAt),
    remote_payload: apiTemplate || null,
    last_synced_at: new Date().toISOString()
  };
}

export async function getBizgoNotificationTemplate({ senderKey, templateCode }) {
  const payload = await bizgoRequest("/v1/center/alimtalk/template", {
    query: {
      senderKey: senderKey || getBizgoSenderKey(),
      templateCode
    }
  });

  return extractAlimtalk(payload);
}

export async function getBizgoMessageInquiry(msgKey) {
  const payload = await bizgoRequest(
    `/v1/message/inquiry/msgKey/${encodeURIComponent(msgKey)}`
  );

  return {
    payload,
    message: getBizgoInquiryMessages(payload)[0] || null
  };
}

export async function getBizgoDeliveryStatusByMsgKey(msgKey) {
  const inquiry = await getBizgoMessageInquiry(msgKey);
  const result = parseBizgoInquiryMessage(inquiry.message);

  return {
    status: result.status,
    message: result.message,
    response: {
      inquiry: inquiry.payload,
      msgKey
    }
  };
}

export async function resolveBizgoNotificationTemplateForSend(notificationTemplate) {
  const storedTemplate = notificationTemplate;

  if (!storedTemplate?.template_code) {
    throw new Error("알림톡 템플릿에 템플릿 코드가 필요합니다.");
  }

  if (canUseApprovedTemplateSnapshot(storedTemplate)) {
    return storedTemplate;
  }

  if (
    storedTemplate.inspection_status &&
    storedTemplate.inspection_status !== "APR" &&
    hasFreshInspectionSnapshot(storedTemplate)
  ) {
    assertApprovedNotificationTemplate(storedTemplate);
  }

  const remoteTemplate = await getBizgoNotificationTemplate({
    senderKey: storedTemplate.sender_key || getBizgoSenderKey(),
    templateCode: storedTemplate.template_code
  });

  if (!remoteTemplate) {
    throw new Error("Bizgo에서 최신 알림톡 템플릿을 조회하지 못했습니다.");
  }

  const syncedInput = mapBizgoTemplateToLocal(remoteTemplate);
  const nextTemplate = {
    ...storedTemplate,
    ...syncedInput
  };

  if (storedTemplate.id) {
    try {
      const updatedTemplate = await updateNotificationTemplate(
        storedTemplate.id,
        {
          status: storedTemplate.status,
          ...syncedInput
        },
        {
          currentTemplate: storedTemplate
        }
      );

      assertApprovedNotificationTemplate(updatedTemplate);

      return {
        ...storedTemplate,
        ...updatedTemplate
      };
    } catch (error) {
      if (error instanceof Error && error.message.includes("알림톡 템플릿이 아직 승인 완료 상태가 아닙니다.")) {
        throw error;
      }

      console.error("알림톡 템플릿 자동 동기화 실패", error);
    }
  }

  assertApprovedNotificationTemplate(nextTemplate);

  return nextTemplate;
}

export async function createBizgoNotificationTemplate(template) {
  const payload = await bizgoRequest("/v1/center/alimtalk/template", {
    method: "POST",
    body: buildNotificationTemplateBody(template)
  });

  return extractAlimtalk(payload);
}

export async function updateBizgoNotificationTemplate(template) {
  const payload = await bizgoRequest("/v1/center/alimtalk/template", {
    method: "PUT",
    body: buildNotificationTemplateBody(template)
  });

  return extractAlimtalk(payload);
}

export async function deleteBizgoNotificationTemplate({ senderKey, templateCode }) {
  return await bizgoRequest(
    `/v1/center/alimtalk/template/senderKey/${encodeURIComponent(senderKey)}/templateCode/${encodeURIComponent(templateCode)}`,
    {
      method: "DELETE"
    }
  );
}

export async function requestBizgoTemplateApproval({
  senderKey,
  senderKeyType,
  templateCode,
  comment
}) {
  return await bizgoRequest("/v1/center/alimtalk/template/request", {
    method: "POST",
    body: {
      alimtalk: {
        senderKey,
        senderKeyType: senderKeyType || "S",
        templateCode,
        comment: toNullableString(comment)
      }
    }
  });
}

export async function cancelBizgoTemplateApproval({ senderKey, templateCode }) {
  return await bizgoRequest("/v1/center/alimtalk/template/request/cancel", {
    method: "POST",
    body: {
      senderKey,
      templateCode
    }
  });
}

export async function sendBizgoAlimtalk({ notificationTemplate, document }) {
  if (!document.recipient_phone) {
    throw new Error("알림톡 발송을 위해 수신 전화번호가 필요합니다.");
  }

  const template = await resolveBizgoNotificationTemplateForSend(notificationTemplate);

  if (!template.message) {
    throw new Error("알림톡 템플릿에 템플릿 코드와 메시지가 모두 필요합니다.");
  }

  const values = templateValuesFromDocument(document);
  const alimtalk = buildBizgoSendAlimtalk(template, values);
  const payload = {
    messageFlow: [
      {
        alimtalk
      }
    ],
    destinations: [
      {
        to: document.recipient_phone,
        replaceWords: sanitizeReplaceWords(values)
      }
    ],
    ref: `document:${document.token}`
  };

  const response = await fetch(buildBizgoUrl("v1/send/omni").toString(), {
    method: "POST",
    headers: getBizgoHeaders(),
    body: JSON.stringify(payload),
    cache: "no-store"
  });

  const data = await response.json().catch(() => null);

  if (!response.ok) {
    throw new Error(`Bizgo 발송 실패: ${response.status} ${JSON.stringify(data)}`);
  }

  assertBizgoSendSuccess(data);
  const destination = getBizgoFirstDestination(data);
  const msgKey = destination?.msgKey || null;
  let inquiryPayload = null;
  let inquiryMessage = null;

  if (msgKey) {
    for (let attempt = 0; attempt < 4; attempt += 1) {
      if (attempt > 0) {
        await sleep(700);
      }

      const inquiry = await getBizgoMessageInquiry(msgKey);
      inquiryPayload = inquiry.payload;
      inquiryMessage = inquiry.message;

      if (inquiryMessage?.reportCode) {
        break;
      }
    }
  }

  const result = classifyBizgoInquiryMessage(inquiryMessage);

  return {
    status: result.status,
    message: result.message,
    response: {
      send: data,
      inquiry: inquiryPayload,
      msgKey
    }
  };
}
