...
export async function resolveBizgoNotificationTemplateForSend(notificationTemplate) {
  ...
  if (storedTemplate.id) {
    try {
      const updatedTemplate = await updateNotificationTemplate(storedTemplate.id, {
        status: storedTemplate.status,
        ...syncedInput
      });

      if (updatedTemplate.inspection_status !== "APR") {
        throw new Error(
          `알림톡 템플릿이 아직 승인 완료 상태가 아닙니다. 현재 상태: ${updatedTemplate.inspection_status || "미확인"}`
        );
      }
  ...
  if (nextTemplate.inspection_status !== "APR") {
    throw new Error(
      `알림톡 템플릿이 아직 승인 완료 상태가 아닙니다. 현재 상태: ${nextTemplate.inspection_status || "미확인"}`
    );
  }
