...
function applyDocumentFilters(query, { branchId, status, createdSince, signedSince } = {}) {
  let nextQuery = query;
...
export async function listDocuments({
  ...
  query = applyDocumentFilters(query, { branchId, status, createdSince, signedSince });
...
export async function countDocuments({ branchId, status, createdSince, signedSince } = {}) {
  const supabase = getSupabaseAdmin();

  return await countRows(() => {
    let query = supabase.from("documents").select("id", { count: "exact", head: true });
    return applyDocumentFilters(query, { branchId, status, createdSince, signedSince });
  }, "문서 수 조회 실패");
}
